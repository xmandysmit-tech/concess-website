...
export default async function StudioCasePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <main style={{ background: "var(--color-linen-100)", minHeight: "100vh" }}>
      ...
      {project.heroImage ? (
        <section style={{ background: "var(--color-linen-100)", minHeight: "100vh", display: "flex", flexDirection: "column" }}>
          {/* Terugknop — altijd links boven */}
          <div className="max-w-5xl mx-auto px-6 md:px-16 w-full pt-20">
          ...
          <div className="max-w-5xl mx-auto px-6 md:px-16 w-full flex-1 flex items-center py-8">
      ...
      {/* ── SHORT-FORM VIDEO GRID (zonder trailer) ── */}
      {project.videos && project.videos.length > 0 && !youtubeId && (
